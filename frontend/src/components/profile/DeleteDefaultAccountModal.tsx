...
import { deleteDefaultAccount } from "../../services/accountService";
...
interface DeleteDefaultAccountModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const DeleteDefaultAccountModal: React.FC<DeleteDefaultAccountModalProps> = ({
  isOpen,
  onClose,
  defaultAccounts,
  accountId,
  ...
  const availableAccounts = defaultAccounts.filter((acc) => acc.id !== accountId);
  ...
  const steps = hasAmount ? ["Transfer Funds", "Confirm Delete"] : ["Confirm Delete"];
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!hasAmount) {
      try {
        await deleteDefaultAccount(accountId);
        setLoading(false);
        onSuccess(accountId);
        handleClose();
      } catch (err) {
        ...
        console.error("Error deleting default account:", err);
      }
      return;
    }
    ...
    try {
      ...
      setLoading(false);
      console.error("Error in transfer and delete:", err);
      setError(
    ...
    if (isTransferStep) {
      return (
        <div className="space-y-4">
          <div className="mb-5 px-4 py-2 bg-blue-50 rounded-xl border border-blue-100">
          ...
          <div>
            ...
            <div ref={accountRef} className="relative">
              <motion.div
                ...
                className="flex items-center bg-white border border-blue-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent transition-all bg-blue-50/50"
              ...
              {showAccountDropdown && (
                <motion.div
                  ...
                  className="absolute z-[100] mt-1 w-full bg-white border border-blue-100 rounded-xl shadow-lg max-h-28 overflow-y-auto"
                >
                  {filteredAccounts.length === 0 ? (
                    ...
                    filteredAccounts
                      ...
                      .map((account, index, filteredArray) => (
                      <motion.div
                        key={account.id}
                        whileHover={{
          ...
          <div>
            ...
            <div>
              ...
              >
                ...
                Type{" "}
                <span className="font-semibold text-red-600 mx-1">Delete</span>{" "}
                to confirm
              </label>
              <motion.input
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        <div className="bg-gradient-to-r from-red-600 to-red-800 relative overflow-hidden">
          ...
          <div
            className={`absolute bg-white/15 rounded-full ${
              isMobileView
                ? "top-1 left-12 w-4 h-4"
                : "top-1 left-14 w-6 h-6"
            }`}
          ></div>

          <div
            className={`${
              isMobileView ? "px-4 py-3" : "px-4 py-3"
            } flex items-center justify-between relative z-10 mb-2`}
          >
            <div className="flex items-center">
              <div
                className={`bg-white rounded-full flex items-center justify-center mr-3 shadow-lg ${
          ...
          {hasAmount && (
            <div
              className={`${
                isMobileView ? "px-4 pb-3" : "px-4 pb-3"
              } relative z-10`}
            >
        ...
        <div
          className={`${
            isMobileView ? "p-3" : "p-4"
          } border-t bg-gray-50/50 backdrop-blur-sm flex justify-between`}
        >
          <motion.button
            onClick={prevStep}
            ...
            whileHover={{
              scale: currentStep === 1 || !hasAmount ? 1 : 1.02,
            }}
            whileTap={{ scale: 0.98 }}
          >
            <ArrowLeft size={14} />
...
export default DeleteDefaultAccountModal;