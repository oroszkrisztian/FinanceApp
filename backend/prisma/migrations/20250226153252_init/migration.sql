...
CREATE TABLE `Transaction` (
    ...
    `savingAccountId` INTEGER NULL,
    `recurringFundId` INTEGER NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    `deletedAt` DATETIME(3) NULL,
...
-- AddForeignKey
ALTER TABLE `Transaction` ADD CONSTRAINT `Transaction_savingAccountId_fkey` FOREIGN KEY (`savingAccountId`) REFERENCES `SavingAccount`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Transaction` ADD CONSTRAINT `Transaction_recurringFundId_fkey` FOREIGN KEY (`recurringFundId`) REFERENCES `RecurringFundAndBill`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
