...
async function createCategory(categoryName, token) {
    ...
    if (!response.ok)
        throw new Error("Failed to create ai category user");
    return response.json();
}
async function getAllCategoriesForUser(token) {
    const response = await fetch("https://financeapp-bg0k.onrender.com/categories/getAllCategoriesForUser", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
    ...
    if (!response.ok)
        throw new Error("Failed to get all categories ai for user");
    return response.json();
}
async function getUserBudgets(userId, token) {
    try {
        const response = await fetch(`https://financeapp-bg0k.onrender.com/budget/getAllUserBudgets`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
...
async function createUserBudgetWithCategories(userId, name, limitAmount, currency, categoryIds, token) {
    const response = await fetch(`https://financeapp-bg0k.onrender.com/createUserBudgetWithCategories`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
...
async function updateUserBudget(userId, budgetId, name, limitAmount, currency, categoryIds, token) {
    const response = await fetch(`https://financeapp-bg0k.onrender.com/budget/updateUserBudget`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
...
async function deleteUserBudget(userId, budgetId, token) {
    const response = await fetch(`https://financeapp-bg0k.onrender.com/budget/deleteUserBudget`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
...
ai.post("/chat", async (c) => {
    try {
        ...
        const { question, accounts, transactions, futureOutgoingPayments, futureIncomingPayments, budgets, } = body;
        if (!question || typeof question !== "string") {
            return c.json({ success: false, error: "Missing or invalid question" }, 400);
        }
...
ai.post("/aiCategorySuggestion", async (c) => {
    try {
        ...
        const body = await c.req.json();
        const { paymentName, paymentAmount, paymentType, currency, description, } = body;
        if (!paymentName || !paymentAmount || !paymentType) {
            return c.json({
                success: false,
                error: "Missing required fields: paymentName, paymentAmount, paymentType",
            }, 400);
        }
...
Existing Categories:
${categoriesData.map((cat) => `- ID: ${cat.id}, Name: "${cat.name}"`).join("\n")}

User's Current Budgets (for context):
${userBudgets && userBudgets.length > 0
            ? userBudgets
                .map((budget) => `- Budget: "${budget.name}" (${budget.limitAmount} ${budget.currency}) - Categories: ${budget.categoryIds?.join(", ") || "None"}`)
                .join("\n")
            : "No existing budgets"}
...
Important: Return only the JSON array, no other text or formatting.`;
        ...
        try {
            ...
            const validSuggestions = suggestions
                ...
                if (suggestion.type === "existing") {
                    return (suggestion.categoryId &&
                        ...
                        categoriesData.some((cat) => cat.id === suggestion.categoryId));
                }
                else if (suggestion.type === "new") {
                    return (typeof suggestion.confidence === "number" &&
                        suggestion.confidence >= 0.1 &&
                        suggestion.confidence <= 1.0 &&
                ...
                description: suggestion.type === "new"
            ...
            for (const category of categoriesData) {
                ...
                let confidence = 0;
                let reason = "";
                if (searchText.includes(categoryNameLower) ||
                ...
                for (const budget of userBudgets) {
                    if (budget.categoryIds && budget.categoryIds.length > 0) {
                        for (const categoryId of budget.categoryIds) {
                            ...
                            if (category &&
                                !fallbackSuggestions.some((s) => s.type === "existing" && s.categoryId === categoryId)) {
                                const budgetNameLower = budget.name.toLowerCase();
                                if (paymentNameLower.includes(budgetNameLower) ||
...
ai.post("/budgetEdit", async (c) => {
    try {
        ...
        const body = await c.req.json();
        const { transactions, categories, budgets, futureOutgoingPayments, futureIncomingPayments, } = body;
        const { genAI: geminiClient } = await initializeGemini();
        const model = geminiClient.getGenerativeModel({
            model: "gemini-2.5-flash-preview-05-20",
        });
            ...
            if (aiResponse && Array.isArray(aiResponse.recommendations)) {
                const validRecommendations = aiResponse.recommendations.filter((rec) => {
                    ...
                    return (rec.name &&
                        ...
                        rec.categoryIds.every((id) => availableCategoryIds.includes(id)) &&
                        (rec.action === "create"
                            ? rec.budgetId === null
                            : rec.action === "update"
                                ? existingBudgetIds.includes(rec.budgetId)
    ...
    try {
        ...
        for (const rec of recommendations) {
            try {
                ...
                switch (rec.action) {
                    case "create":
                        result = await createUserBudgetWithCategories(userId, rec.name, rec.limitAmount, rec.currency, rec.categoryIds, token);
                        results.push({
                            success: true,
                            action: "create",
                            name: rec.name,
                        ...
                        break;
                    case "update":
                        result = await updateUserBudget(userId, rec.budgetId, rec.name, rec.limitAmount, rec.currency, rec.categoryIds, token);
                        results.push({
                            success: true,
                            action: "update",
                            name: rec.name,
